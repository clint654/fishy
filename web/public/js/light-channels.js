...
d3.json("/channelprog/" + profile, function (error, json) {
    ...
    d3.json("/channels/", function (error, json) {
        if (error) return console.warn(error);
        json.forEach(function (d) {
            channeldata[d.id] = { name: d.name, class: d.class };
        });
...
function dragged(d) {
    ...
    if (scalex.invert(nx) < parseTime("00:00:00")) { nx = scalex(parseTime("00:00:00")) };
    if (scalex.invert(nx) > parseTime("24:00.00")) { nx = scalex(parseTime("24:00.00")) };
...
function updatedot(i) {
    ...
    if (svg.select("#channeldots" + i)) { console.log("new " + i); svg.append("g").attr("id", "channeldots" + i) }
